...
public class tableDeliveryController implements Initializable {
    ...
    @FXML
    private TableView<MainData> deliveryTable;
    @FXML
    ...
    public void Refresh () {
        ...
        deliveryTable.setItems(this.data);
    }
    ...
    public void deleteDelivery () {
        String sql = "DELETE FROM delivery WHERE ID="+IDTF.getText()+"";
        try {
            Connection conn = dbConnection.getConnection();
            assert conn != null;
        ...
        alert.setContentText("delivery Deleted!");
        alert.showAndWait();
    }

    @FXML
    private void addDelivery () {
        String sql = "INSERT INTO `delivery`(`ID`, `name`, `datta`, `total`, `taxes`) VALUES (? , ?, ?, ?, ?)";
        try {
            Connection conn = dbConnection.getConnection();
            assert conn != null;
            PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    private void updateDelivery ( ActionEvent event ) {
        String sql = "UPDATE delivery  SET name='"+nameTF.getText()+"', datta='"+dateTF.getText()+"', total='"+totalTF.getText()+"', taxes='"+taxesTF.getText()+"' WHERE ID="+IDTF.getText()+"";
        try {
            Connection conn = dbConnection.getConnection();
            assert conn != null;
            PreparedStatement stmt = conn.prepareStatement(sql);
...
//        IDTF.setText(deliveryTable.getSelectionModel().getSelectedItem().getID());                           //1
            MainData cd = deliveryTable.getSelectionModel().getSelectedItem();                                //2
