...
public class tableInvoiceController implements Initializable {
    ...
    @FXML
    public TableView <MainData> invoiceTable;
    @FXML
    ...
    public void Refresh () {
        ...
        invoiceTable.setItems(this.data);
    }
    ...
    public void deleteInvoice () {
        String sql = "DELETE FROM invoice WHERE ID="+IDTF.getText()+"";
        try {
            Connection conn = dbConnection.getConnection();
            assert conn != null;
        ...
        alert.setContentText("invoice Deleted!");
        alert.showAndWait();
    }

    @FXML
    private void addInvoice () {
        String sql = "INSERT INTO `invoice`(`ID`, `name`, `datta`, `total`, `taxes`) VALUES (? , ?, ?, ?, ?)";
        try {
            Connection conn = dbConnection.getConnection();
            assert conn != null;
            PreparedStatement stmt = conn.prepareStatement(sql);
        ...
        alert.setTitle("Success");
        alert.setHeaderText(null);
        alert.setContentText("invoice Added!");
        alert.showAndWait();
    }

    @FXML
    private void updateInvoice ( ActionEvent event ) {
        String sql = "UPDATE invoice  SET name='"+nameTF.getText()+"', datta='"+dateTF.getText()+"', total='"+totalTF.getText()+"', taxes='"+taxesTF.getText()+"' WHERE ID="+IDTF.getText()+"";
        try {
            Connection conn = dbConnection.getConnection();
            assert conn != null;
            PreparedStatement stmt = conn.prepareStatement(sql);
        ...
        alert.setTitle("Success");
        alert.setHeaderText(null);
        alert.setContentText("invoice Updated!");
        alert.showAndWait();
    }

    private void populateTextfields () {
        invoiceTable.setOnMouseClicked(event -> {
//        IDTF.setText(invoiceTable.getSelectionModel().getSelectedItem().getID());                           //1
            MainData cd = invoiceTable.getSelectionModel().getSelectedItem();                                //2
